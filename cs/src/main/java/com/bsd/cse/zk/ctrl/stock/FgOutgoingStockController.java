...
import com.bsd.cse.zk.listbox.renderer.FgOutgoingRenderer;
 ...
 * @author bento
 */
public class FgOutgoingStockController extends SecurityController{
    private static final long serialVersionUID = -3993123706258278544L;
    private static Log LOG = LogFactory.getLog(FgOutgoingStockController.class);
    private final static Long REQUES_USER_PERMISSION = 1030203L;
    ...
    private Label quantiyInStockLbl;
    private Label availableStockLbl;
    ...
    private Listheader transferClm;
...
//        ZKCatalogs.setCustomers(customerCbox, customers);
        ...
        transferTypes = TransferTypeConstant.getOutgoingTypes(MaterialTypeConstant.FG_TYPE_ID);
...
//                HashMap<String,Object> metaData = core.getFgOutgoingTransactions(startDate, endDate, part);
//                if(((Integer)metaData.get("allRecords")).equals(new Integer(0)))
//                {
//                    ArrayList<String> results = new ArrayList<String>();
//                    results.add(ResourceUtil.getLabel("stock.finishgood.outgoing.page.message.norecordfound"));
...
//                }
//
//                contentListbox.setItemRenderer(new FgOutgoingRenderer());
                ...
                GridSearchResult<FgStockTran> result = searchObject.getSearchResult(0, outgoingPg.getPageSize());
                if(result != null && result.getCount()>0)
                {
                    contentListbox.setModel(new ListboxListWrapper<FgStockTran>(contentListbox, outgoingPg, result, searchObject));
                }
                else
                {
                    List<String> noRecords = new ArrayList<String>();
                    noRecords.add(ResourceUtil.getLabel("stock.finishgood.outgoing.page.message.norecordfound"));
                    contentListbox.setModel(new ListboxListWrapper<FgStockTran>(contentListbox, outgoingPg, new ListModelList(noRecords), searchObject));
                }
        ...
        partCbox.addEventListener(Events.ON_CHANGE, new EventListener() {
            ...
            public void onEvent(Event event) throws Exception {                
                ...
                quantiyInStockLbl.setValue(String.valueOf(part.getAmount()));
                customerLbl.setValue(part.getCustomer().getName());
                if(quantityTxt.getValue()!= null)
                {
                    availableStockLbl.setValue(String.valueOf(part.getAmount()-quantityTxt.getValue()));
                }
                else
                {
                    availableStockLbl.setValue(String.valueOf(part.getAmount()));
                }
            }
        });
        ...
        newBtn.setVisible(Executions.getCurrent().isUserInRole("1030201"));
        ...
        newBtn.addEventListener(Events.ON_CLICK, new EventListener() {
            ...
            public void onEvent(Event event) throws Exception {
                ...
                partCbox.getChildren().clear();
                PartCore partCore = new PartCore();
                parts = partCore.getAllList();
                ZKCatalogs.setParts(partCbox, parts,"partName-partNo",null);
                
                customerLbl.setValue(_applied);
        ...
        saveBtn.addEventListener(Events.ON_CLICK, new EventListener() {
            ...
            public void onEvent(Event event) throws Exception {
                ...
                if(quantity == null)
                {
                    AlertMessages.alertMessage("stock.finishgood.outgoing.page.alert.message.4");
                    return;
                }

                if(transferTypeCbox.getSelectedItem() == null)
                {
                    AlertMessages.alertMessage("stock.finishgood.outgoing.page.alert.message.5");
                    return;
                }

                if(partCbox.getSelectedItem() == null)
                {
                    AlertMessages.alertMessage("stock.finishgood.outgoing.page.alert.message.6");
                    return;
                }

                if(requesterCbox.getSelectedItem()  == null)
                {
                    AlertMessages.alertMessage("stock.finishgood.outgoing.page.alert.message.7");
                    return;
                }
                ...
                if(part.getAmount() < quantity)
                {
                    AlertMessages.alertMessage("stock.finishgood.outgoing.page.alert.message.8");
                    return;
                }
                ...
                try
                {
                    ...
                    {                        
                        ...
                        AlertMessages.successMessage("stock.finishgood.outgoing.page.alert.message.1");
                        Events.sendEvent(Events.ON_CLICK, searchBtn, null);
                        event.stopPropagation();
                    }
                    else if(result != null && !result)
                    {
                        AlertMessages.failMessage("stock.finishgood.outgoing.page.alert.message.2");
                        return;
                    }
                }
                catch(Exception e)
                {
                    ...
                    {
                        AlertMessages.failMessage("stock.finishgood.outgoing.page.alert.message.3");
                    }
                    else
                    {
    ...
    public void createComponent(Component component)
    {
        ...
        quantiyInStockLbl = (Label)getComponent(editWin, "quantiyInStockLbl", quantiyInStockLbl);
        availableStockLbl = (Label)getComponent(editWin, "availableStockLbl", availableStockLbl);
    ...
    {
        transferClm.setSortAscending(new FieldComparator("transferType.transferTypeName", false));
        transferClm.setSortDescending(new FieldComparator("transferType.transferTypeName", true));
