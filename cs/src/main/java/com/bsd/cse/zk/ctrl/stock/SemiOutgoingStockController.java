...
import com.bsd.cse.zk.listbox.renderer.SemiOutgoingRenderer;
 ...
 * @author bento
 */
public class SemiOutgoingStockController extends SecurityController{
    private static final long serialVersionUID = -3993123706258278544L;
    private static Log LOG = LogFactory.getLog(SemiOutgoingStockController.class);
    private final static Long REQUES_USER_PERMISSION = 1030603L;
    ...
    public void initialData() throws Exception
    {
        ...
        transferTypes = TransferTypeConstant.getOutgoingTypes(MaterialTypeConstant.SEMI_TYPE_ID);
        ...
        ZKCatalogs.setSemiMaterials(semiMaterialSearchCbox, semiMaterials,"semiName-semiType",null);
...
//                HashMap<String,Object> metaData = core.getSemiOutgoingTransactions(startDate, endDate,part, semiMaterial, transferType);
//                if(((Integer)metaData.get("allRecords")).equals(new Integer(0)))
//                {
//                    ArrayList<String> results = new ArrayList<String>();
//                    results.add(ResourceUtil.getLabel("stock.semimaterial.outgoing.page.message.norecordfound"));
...
//                }
                ...
                contentListbox.setItemRenderer(new SemiOutgoingRenderer());
                ...
                GridSearchResult<SemiStockTran> result = searchObject.getSearchResult(0, outgoingPg.getPageSize());
                if(result != null && result.getCount()>0)
                {
                    contentListbox.setModel(new ListboxListWrapper<SemiStockTran>(contentListbox, outgoingPg, result, searchObject));
                }
                else
                {
                    List<String> noRecords = new ArrayList<String>();
                    noRecords.add(ResourceUtil.getLabel("stock.semimaterial.outgoing.page.message.norecordfound"));
                    contentListbox.setModel(new ListboxListWrapper<SemiStockTran>(contentListbox, outgoingPg, new ListModelList(noRecords), searchObject));
                }
        ...
        partCbox.addEventListener(Events.ON_CHANGE, new EventListener() {
            ...
            public void onEvent(Event event) throws Exception {                                
                ...
                SemiMaterial semiMaterial  = core.getSemiMaterial(part.getSemiMaterial().getId());
                semiMaterialLbl.setValue(semiMaterial.getSemiName()+"-"+semiMaterial.getSemiType().getName());
                quantityInStockLbl.setValue(String.valueOf(semiMaterial.getAmount()));
                if(quantityTxt.getValue() != null)
                {
                    availableInStockLbl.setValue(String.valueOf(semiMaterial.getAmount()-quantityTxt.getValue()));
                }
                else
                {
                    availableInStockLbl.setValue(String.valueOf(semiMaterial.getAmount()));
                }
                sizeLbl.setValue(String.valueOf(semiMaterial.getSize()));
               
            }
        });


        newBtn.setVisible(Executions.getCurrent().isUserInRole("1030601"));
        ...
        saveBtn.addEventListener(Events.ON_CLICK, new EventListener() {
            ...
            public synchronized void onEvent(Event event) throws Exception {
                ...
                if(quantity == null)
                {
                    AlertMessages.alertMessage("stock.semimaterial.outgoing.page.alert.message.4");
                    return;
                }               

                if(partCbox.getSelectedItem() == null)
                {
                    AlertMessages.alertMessage("stock.semimaterial.outgoing.page.alert.message.5");
                    return;
                }

                if(requesterCbox.getSelectedItem()  == null)
                {
                    AlertMessages.alertMessage("stock.semimaterial.outgoing.page.alert.message.6");
                    return;
                }

                if(transferTypeCbox.getSelectedItem()  == null)
                {
                    AlertMessages.alertMessage("stock.semimaterial.outgoing.page.alert.message.7");
                    return;
                }
                ...
                SemiMaterialCore SemiMaterialcore = new SemiMaterialCore();
                SemiMaterial semiMaterial  = SemiMaterialcore.getSemiMaterial(part.getSemiMaterial().getId());
                ...
                if(semiMaterial.getAmount() < quantity)
                {
                    AlertMessages.alertMessage("stock.semimaterial.outgoing.page.alert.message.8");
                    return;
                }
                ...
                try
                {
                    ...
                    {                        
                        ...
                        AlertMessages.successMessage("stock.semimaterial.outgoing.page.alert.message.1");
                        Events.sendEvent(Events.ON_CLICK, searchBtn, null);
                        event.stopPropagation();
                    }
                    else if(result != null && !result)
                    {
                        AlertMessages.failMessage("stock.semimaterial.outgoing.page.alert.message.2");
                        return;
                    }
                }
                catch(Exception e)
                {
                    ...
                    {
                        AlertMessages.failMessage("stock.semimaterial.outgoing.page.alert.message.3");
                    }
                    else
                    {
