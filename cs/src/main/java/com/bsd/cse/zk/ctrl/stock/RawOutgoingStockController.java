...
import com.bsd.cse.app.stock.RawStockCore;
...
import com.bsd.cse.zk.listbox.renderer.RawIncomingRenderer;
import com.bsd.cse.zk.listbox.renderer.RawOutgoingRenderer;
 ...
 * @author bento
 */
public class RawOutgoingStockController extends SecurityController{
    private static final long serialVersionUID = -3993123706258278544L;
    private static Log LOG = LogFactory.getLog(RawOutgoingStockController.class);
    private final static Long REQUES_USER_PERMISSION = 1030403L;
    ...
    private Combobox rawMaterialSearchCbox;
    ...
    private Label quantityInStockLbl;
    private Label availableInStockLbl;
    private Label sizeLbl;
    private Label rawMaterialLbl;
    private Listbox contentListbox;                      
    private List<TransferType> transferTypes;
    private List<Part> parts;
    private List<RawMaterial> rawMaterials;
    private List<User> requesters;
    private Listheader outgoingTypeClm;
    private Listheader partClm;
    private Listheader rawMaterialClm;
    ...
    {               
        ...
        transferTypes = TransferTypeConstant.getOutgoingTypes(MaterialTypeConstant.RAW_TYPE_ID);

        PartCore partCore = new PartCore();
        parts = partCore.getAllList();
        ZKCatalogs.setParts(partCbox, parts,"partName-PartNo",null);
        
        RawMaterialCore rawMaterialCore = new RawMaterialCore();
        rawMaterials = rawMaterialCore.getAllList();
        ...
        ZKCatalogs.setRawMaterials(rawMaterialSearchCbox, rawMaterials,"rawName-rawType",null);
        Comboitem rawMaterialSearchItem = new Comboitem();
        rawMaterialSearchItem.setLabel("All");
        List<Comboitem> rawMaterialSearchItems =  rawMaterialSearchCbox.getItems();
        rawMaterialSearchItems.add(0, rawMaterialSearchItem);
        rawMaterialSearchCbox.setSelectedIndex(0);
    ...
    {
        CriteriaObject criteriaObject = new CriteriaObject(RawStockTran.class);
        ...
        criteriaObject.addAliasNames("semiMaterial.rawMaterial", "rawMaterial",CriteriaObject.LEFT_JOIN);
        ...
        if(rawMaterialSearchCbox.getSelectedItem() != null)
        {
            RawMaterial rawMaterial = (RawMaterial)rawMaterialSearchCbox.getSelectedItem().getValue();
            if(rawMaterial != null)
            {
                criteriaObject.addCriteria(Restrictions.eq("rawMaterial.id",rawMaterial.getId()));
            }
        }



        criteriaObject.addCriteria(Restrictions.eq("transferType.stockType.id",StockTypeConstant.OUTGOING_TYPE_ID));
...
//                TransferType transferType = (TransferType)transferTypeSearchCbox.getSelectedItem().getValue();
//                RawMaterial rawMaterial = (RawMaterial)rawMaterialSearchCbox.getSelectedItem().getValue();
//                RawStockCore core = new RawStockCore();
//                HashMap<String,Object> metaData = core.getRawOutgoingTransactions(startDate, endDate,part, rawMaterial, transferType);
//                if(((Integer)metaData.get("allRecords")).equals(new Integer(0)))
//                {
//                    ArrayList<String> results = new ArrayList<String>();
//                    results.add(ResourceUtil.getLabel("stock.rawmaterial.outgoing.page.message.norecordfound"));
...
//                }
//
//                contentListbox.setItemRenderer(new RawOutgoingRenderer());

                outgoingPg.setPageSize(PagingConstant.PAGING_SIZE);
                outgoingPg.setDetailed(Boolean.TRUE);
                HibernateSearchObject<RawStockTran> searchObject = new HibernateSearchObject<RawStockTran>(createCriteria());
                GridSearchResult<RawStockTran> result = searchObject.getSearchResult(0, outgoingPg.getPageSize());
                if(result != null && result.getCount()>0)
                {
                    contentListbox.setModel(new ListboxListWrapper<RawStockTran>(contentListbox, outgoingPg, result, searchObject));
                }
                else
                {
                    List<String> noRecords = new ArrayList<String>();
                    noRecords.add(ResourceUtil.getLabel("stock.rawmaterial.outgoing.page.message.norecordfound"));
                    contentListbox.setModel(new ListboxListWrapper<RawStockTran>(contentListbox, outgoingPg, new ListModelList(noRecords), searchObject));
                }
        ...
        partCbox.addEventListener(Events.ON_CHANGE, new EventListener() {
            ...
            public void onEvent(Event event) throws Exception {                                
                ...
                RawMaterial rawMaterial  = core.getRawMaterial(part.getSemiMaterial().getRawMaterial().getId());
                rawMaterialLbl.setValue(rawMaterial.getRawName()+"-"+rawMaterial.getRawType().getName());
                quantityInStockLbl.setValue(String.valueOf(rawMaterial.getAmount()));
                if(quantityTxt.getValue() != null)
                {
                    availableInStockLbl.setValue(String.valueOf(rawMaterial.getAmount()-quantityTxt.getValue()));
                }
                else
                {
                    availableInStockLbl.setValue(String.valueOf(rawMaterial.getAmount()));
                }
                sizeLbl.setValue(String.valueOf(rawMaterial.getSize()));
               
            }
        });

        newBtn.setVisible(Executions.getCurrent().isUserInRole("1030401"));
        ...
        quantityTxt.addEventListener(Events.ON_CHANGE, new EventListener() {
        ...
        saveBtn.addEventListener(Events.ON_CLICK, new EventListener() {
            ...
            public synchronized void onEvent(Event event) throws Exception {
                ...
                RawStockTran tran = new RawStockTran();
                ...
                if(quantity == null)
                {
                    AlertMessages.alertMessage("stock.rawmaterial.outgoing.page.alert.message.4");
                    return;
                }               

                if(partCbox.getSelectedItem() == null)
                {
                    AlertMessages.alertMessage("stock.rawmaterial.outgoing.page.alert.message.5");
                    return;
                }

                if(requesterCbox.getSelectedItem()  == null)
                {
                    AlertMessages.alertMessage("stock.rawmaterial.outgoing.page.alert.message.6");
                    return;
                }

                if(transferTypeCbox.getSelectedItem()  == null)
                {
                    AlertMessages.alertMessage("stock.rawmaterial.outgoing.page.alert.message.7");
                    return;
                }
                ...
                RawMaterialCore rawMaterialcore = new RawMaterialCore();
                RawMaterial rawMaterial  = rawMaterialcore.getRawMaterial(part.getSemiMaterial().getRawMaterial().getId());
                ...
                if(rawMaterial.getAmount() < quantity)
                {
                    AlertMessages.alertMessage("stock.rawmaterial.outgoing.page.alert.message.8");
                    return;
                }
                ...
                try
                {
                    ...
                    {                        
                        ...
                        AlertMessages.successMessage("stock.rawmaterial.outgoing.page.alert.message.1");
                        Events.sendEvent(Events.ON_CLICK, searchBtn, null);
                        event.stopPropagation();
                    }
                    else if(result != null && !result)
                    {
                        AlertMessages.failMessage("stock.rawmaterial.outgoing.page.alert.message.2");
                        return;
                    }
                }
                catch(Exception e)
                {
                    ...
                    {
                        AlertMessages.failMessage("stock.rawmaterial.outgoing.page.alert.message.3");
                    }
                    else
                    {
    ...
    public void createComponent(Component component)
    {
        ...
        quantityInStockLbl = (Label)getComponent(editWin, "quantityInStockLbl", quantityInStockLbl);
        availableInStockLbl = (Label)getComponent(editWin, "availableInStockLbl", availableInStockLbl);
        rawMaterialLbl = (Label)getComponent(editWin, "rawMaterialLbl", rawMaterialLbl);
        partSearchCbox = (Combobox)getComponent(component, "partSearchCbox", partSearchCbox);
        rawMaterialSearchCbox= (Combobox)getComponent(component, "rawMaterialSearchCbox", rawMaterialSearchCbox);
        ...
        outgoingPg = (Paging)getComponent(component, "outgoingPg", outgoingPg);
    }

    private void setSortFieldGrid()
    {
        outgoingTypeClm.setSortAscending(new FieldComparator("transferType.transferTypeName", false));
        outgoingTypeClm.setSortDescending(new FieldComparator("transferType.transferTypeName", true));
        ...
        rawMaterialClm.setSortAscending(new FieldComparator("rawMaterial.rawName", false));
        rawMaterialClm.setSortDescending(new FieldComparator("rawMaterial.rawName", true));
        sizeClm.setSortAscending(new FieldComparator("rawMaterial.size", false));
        sizeClm.setSortDescending(new FieldComparator("rawMaterial.size", true));
