 ...
 * @author bento
 */
public class RawIncomingStockController extends SecurityController{
    private static final long serialVersionUID = -3993123706258278544L;
    private static Log LOG = LogFactory.getLog(RawIncomingStockController.class);
    ...
    {               
        ...
        transferTypes = TransferTypeConstant.getIncomingTypes(MaterialTypeConstant.RAW_TYPE_ID);
...
//                HashMap<String,Object> metaData = core.getRawIncomingTransactions(startDate, endDate,part, rawMaterial, transferType);
//                if(((Integer)metaData.get("allRecords")).equals(new Integer(0)))
//                {
//                    ArrayList<String> results = new ArrayList<String>();
//                    results.add(ResourceUtil.getLabel("stock.rawmaterial.incoming.page.message.norecordfound"));
...
//                }
//
//                contentListbox.setItemRenderer(new RawIncomingRenderer());
                ...
                GridSearchResult<RawStockTran> result = searchObject.getSearchResult(0, incomingPg.getPageSize());
                if(result != null && result.getCount()>0)
                {
                    contentListbox.setModel(new ListboxListWrapper<RawStockTran>(contentListbox, incomingPg, result, searchObject));
                }
                else
                {
                    List<String> noRecords = new ArrayList<String>();
                    noRecords.add(ResourceUtil.getLabel("stock.rawmaterial.incoming.page.message.norecordfound"));
                    contentListbox.setModel(new ListboxListWrapper<RawStockTran>(contentListbox, incomingPg, new ListModelList(noRecords), searchObject));
                }
        ...
        partCbox.addEventListener(Events.ON_CHANGE, new EventListener() {
            ...
            public void onEvent(Event event) throws Exception {                                
                ...
                rawMaterialLbl.setValue(part.getSemiMaterial().getRawMaterial().getRawName()+"-"+part.getSemiMaterial().getRawMaterial().getRawType().getName());
                sizeLbl.setValue(String.valueOf(part.getSemiMaterial().getRawMaterial().getSize()));
               
            }
        });

        newBtn.setVisible(Executions.getCurrent().isUserInRole("1030301"));
        ...
        saveBtn.addEventListener(Events.ON_CLICK, new EventListener() {
            ...
            public synchronized void onEvent(Event event) throws Exception {
                ...
                if(quantity == null)
                {
                    AlertMessages.alertMessage("stock.rawmaterial.incoming.page.alert.message.4");
                    return;
                }               

                if(partCbox.getSelectedItem() == null)
                {
                    AlertMessages.alertMessage("stock.rawmaterial.incoming.page.alert.message.5");
                    return;
                }

                if(requesterCbox.getSelectedItem()  == null)
                {
                    AlertMessages.alertMessage("stock.rawmaterial.incoming.page.alert.message.6");
                    return;
                }

                if(transferTypeCbox.getSelectedItem()  == null)
                {
                    AlertMessages.alertMessage("stock.rawmaterial.incoming.page.alert.message.7");
                    return;
                }
                ...
                try
                {
                    ...
                    {                        
                        ...
                        AlertMessages.successMessage("stock.rawmaterial.incoming.page.alert.message.1");
                        Events.sendEvent(Events.ON_CLICK, searchBtn, null);
                        event.stopPropagation();
                    }
                    else if(result != null && !result)
                    {
                        AlertMessages.failMessage("stock.rawmaterial.incoming.page.alert.message.2");
                        return;
                    }
                }
                catch(Exception e)
                {
                    ...
                    {
                        AlertMessages.failMessage("stock.rawmaterial.incoming.page.alert.message.3");
                    }
                    else
                    {
