    ...
    extends DatasourceAdminReportAction
{
    ...
    protected static Log log = LogFactory.getLog(Report08Action.class);
    private static final long serialVersionUID = -2634117399142812092L;

    public Report08Action()
    {
...
//            Date endTime = format.parse(getEndDate());
            ...
            List<OEEData> oeeDatas = processor.processor(startTime, null, null,new Long(getEmployeeId()),null);
            for(OEEData oeeData : oeeDatas)
            {
                datasources.add(oeeData);
            }
        } catch (Exception ex) {
            Logger.getLogger(Report08Action.class.getName()).log(Level.SEVERE, null, ex);
        }
